...
public class CubeColoring : MonoBehaviour {
	...
	void Update () {
		vector.x  = vector.x + Time.deltaTime / 2000;
		vector.y  = vector.y + Time.deltaTime / 2000;
		vector.z  = vector.z + Time.deltaTime / 2000;
		getRotatingMatrixForZ (Time.deltaTime);

		Debug.Log ("Vector - X : " + (float)vector.x + " Y : " + vector.y + " Z : " + vector.z);
		...
		Debug.Log ("Transform - X : " + (float)result [0, 0] + " Y : " + (float)result [1, 0] + " Z : " + (float)result [2, 0]);
	}
